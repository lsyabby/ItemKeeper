...
class InStock3ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, ZHDropDownMenuDelegate {

    @IBOutlet weak var filterDropDownMenu: ZHDropDownMenu!
    @IBOutlet weak var instock3TableView: UITableView!
    ...
    override func viewDidLoad() {
        ...
        instock3TableView.delegate = self
        instock3TableView.dataSource = self

        let nib = UINib(nibName: "ItemListTableViewCell", bundle: nil)
        instock3TableView.register(nib, forCellReuseIdentifier: "ItemListTableCell")
        ...
        self.ref.child("items/mxI0h7c9GlR1eVZRqH8Sfs1LP6B2").observeSingleEvent(of: .value) { (snapshot) in
            ...
            self.items = allItems
            self.instock3TableView.reloadData()
        }
    }
